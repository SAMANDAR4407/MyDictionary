...
class FavouritesAdapter(private var cursor: Cursor) :
    RecyclerView.Adapter<FavouritesAdapter.ViewHolder>(), TextToSpeech.OnInitListener {
    var isUzbek = false
    private var isValid = false
    private var listenerStar: ((Long) -> Unit?)? = null
    private var itemClickListener: ((Context) -> Unit?)? = null

    private var tt: TextToSpeech? = null

    fun setStarListener(block: (Long) -> Unit) {
        listenerStar = block
    }
